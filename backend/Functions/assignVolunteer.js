...
const haversine = require("../Functions/haversine")
...
const assign = async (order_id, order_location, assigned_volunteer) => {
  ...
  ORDER.findByIdAndUpdate(order_id, {
    ...
    .then((result) => {
      ...
      const msg = `Hey,${assigned_volunteer.name}. You've been assigned a/an ${
        result.order_type
      } ${result._id
        .toString()
        .slice(-4)}. Login to the औषध-Setu website to accept it.`;
  ...
  if (order_location === null) {
    ...
    const volunteerDistanceList = await Promise.all(
      volunteerList.map(async (vol) => {
        ...
        // Use Haversine to find the the distance between user and volunteer

        // const distance = haversine(lat_vol, lng_vol, lat_or, lng_or)
